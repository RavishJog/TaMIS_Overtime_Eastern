...
//        takeScreenShot(scenario);
        try {
            WebElement element = w.until(ExpectedConditions.presenceOfElementLocated(
                    By.xpath("//p[@class='menu-label'][contains(.,'Human Resource')]")));
            element.click(); // optional if you want to validate the click
        } catch (Exception e) {
            System.out.println("Success Login failed: " + e.getMessage());
            takeScreenShot(scenario);  // ✅ called only on failure
            Assert.fail("Success Login failed");
        }
    ...
    public void iClickOnHumanResource() throws InterruptedException {
...
//        driver.findElement(By.xpath("//p[@class='menu-label'][contains(.,'Human Resource')]")).click();
        ...
        WebElement element = w.until(ExpectedConditions.elementToBeClickable(By.xpath("//p[contains(.,'Human Resource')]")
        ));
...
// Perform hover action
    ...
    public void iClickOnRetrieveAttendance() {
        WebDriverWait w = new WebDriverWait(driver, 30);
        WebElement element = w.until(ExpectedConditions.elementToBeClickable(By.xpath("//span[@class='ui-menuitem-text'][contains(.,'Retrieve Attendance')]")));
        try {
...
//        }
        ...
        try {
            ...
            WebElement label = w.until(ExpectedConditions.elementToBeClickable(
                    By.xpath("//label[contains(.,'- All Employees -')]")));

            // Scroll into view (to prevent interception)
            ((JavascriptExecutor) driver).executeScript("arguments[0].scrollIntoView(true);", label);
            Thread.sleep(300); // Let UI settle (animations, overlays, etc.)
            ...
            WebElement label = driver.findElement(By.xpath("//label[contains(.,'- All Employees -')]"));
            ((JavascriptExecutor) driver).executeScript("arguments[0].click();", label);
        }

        try {
            // Wait for the input field to appear
            WebElement input = w.until(ExpectedConditions.visibilityOfElementLocated(
                    By.xpath("//input[contains(@id,'selectDate:empCode_filter')]")));

            // Enter the employee code and press Enter
            input.sendKeys(Emp_Code + Keys.ENTER);
        } catch (Exception e) {
            System.out.println("Sending Emp Code failed: " + e.getMessage());
            Assert.fail("Sending Emp Code failed");
        }
...
// --- Off Leave Hours (OLH) ---
    ...
    public void iSignOut() {
        ...
        WebDriverWait w = new WebDriverWait(driver, 30);
        WebElement element = w.until(ExpectedConditions.elementToBeClickable(By.xpath("//a[contains(.,'Sign Out')]")));
        try {
...
//        }
        ...
        try {
            // Wait for the input field to appear
            WebElement input = w.until(ExpectedConditions.visibilityOfElementLocated(
                    By.xpath("//input[@id='empCode_filter']")));
